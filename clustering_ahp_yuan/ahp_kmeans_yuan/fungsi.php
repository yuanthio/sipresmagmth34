...
function getKriteriaNama($no_urut)
{
	...
	$nama = array(); // Inisialisasi array untuk menghindari masalah variabel yang tidak terdefinisi

	while ($row = mysqli_fetch_array($result)) {
		$nama[] = $row['nama'];
	}

	// Periksa apakah $no_urut adalah indeks yang valid
...
function tambahData($tabel, $nama)
{
	...
	$checkQuery = "SELECT * FROM $tabel WHERE nama = '$nama'";
	$checkResult = mysqli_query($koneksi, $checkQuery);

	if (!$checkResult) {
		echo "Error checking duplicate data";
		exit();
	}

	if (mysqli_num_rows($checkResult) > 0) {
		// Data already exists, display an alert or handle it as needed
		echo '<script>alert("Data already exists. Cannot add duplicate entries.");</script>';
		return;
	}
						...
						<tr>
							...
							<td>
								<div class="field">
									...
									?>
									<input type="number" name="bobot<?php echo $urut ?>" value="<?php echo $nilai ?>" required>
								</div>
							</td>
						</tr>
						<?php
